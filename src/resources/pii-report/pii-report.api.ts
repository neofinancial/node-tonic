...
import { CancelPiiReportParams, JobScanModel, MostRecentActiveOrCompletedParams, PiiReportAPI, PiiReportAPIRequestArgsCreator, PiiReportParams, PiiReportRun, StartPiiReportParams } from './pii-report.api.types';

const piiReportApiRequestArgsCreator = (configuration: Configuration): PiiReportAPIRequestArgsCreator => {
  return {
    getGetMostRecentActiveOrCompletedStatusRequestArgs: async (params: MostRecentActiveOrCompletedParams,options?: AxiosRequestConfig): Promise<RequestArgs> => getAxiosRequestArgs('GET', `/api/PiiReport/most_recent_active_or_completed_status`, params, configuration, options),
    getPiiReportRequestArgs: async (params: PiiReportParams, options?: AxiosRequestConfig): Promise<RequestArgs> =>  getAxiosRequestArgs('GET', `/api/PiiReport`, params, configuration, options),
    getStartPiiReportRequestArgs: async (params: StartPiiReportParams, options?: AxiosRequestConfig): Promise<RequestArgs> =>  getAxiosRequestArgs('POST', `/api/PiiReport/start`, params, configuration, options),
    getCancelPiiReportRequestArgs: async (params: CancelPiiReportParams, options?: AxiosRequestConfig): Promise<RequestArgs> => getAxiosRequestArgs('POST', `/api/PiiReport/cancel`, params, configuration, options),
  };
};

const piiReportApi = (configuration:Configuration): Partial<PiiReportAPI> => {
  return {
    async getMostRecentActiveOrCompletedPiiReport(
      ...
      const requestAxiosArgs = await piiReportApiRequestArgsCreator(configuration).getGetMostRecentActiveOrCompletedStatusRequestArgs(
        params,
        options
      );
